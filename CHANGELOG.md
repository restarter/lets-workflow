...
### Added
- **`--session`: bank a snapshot without ending or compacting anything (lets-ouy1l).** `--pre-compact` was the only snapshot-only path, so recording the state of a long session meant writing a file that says it was made before a `/compact` - a small lie in a permanent record that `/lets:start` reads back. `/lets:end --session` and `/lets:note --session` (alias `--snapshot` on both) write the same recovery-grade snapshot, branded honestly, with no settlement: no commit, push, progress or finish offers, and the session continues. The two flags now differ only in what the record says it is for. The `--session` snapshot also carries a `### Range` block, which `--pre-compact` never had, because the boundary reader moved out of `/lets:end` into a new internal `session-boundary` skill: `end` needs it early to gate its own offers and `session-snapshot` needs it late for the block it writes, and two copies of the ladder that fixed lets-370mx would drift apart. `--pre-compact` keeps its lean contract - no boundary read on the path taken immediately before a `/compact`.

### Changed
- **`/lets:end` refers an open task to `/lets:done` instead of finishing it (lets-mqye8).** `end` opened by declaring it was not task completion and then implemented a slice of it: an option asserting "work looks complete" inferred from a commit count, and a paragraph enumerating what `done` supersedes - `end` modelling `done`'s internals, which goes stale the moment `done` changes and which nothing catches. The offer now rests on a status fact plus observable activity: the task is open, and this session committed, dirtied or got ahead. Whether the work is actually finished stays with `done`'s own Scope Verification, which reads the task description rather than counting commits. The hand-off collapses to one rule, refer and stop, and the supersession matrix, the worktree-hint branch that existed to serve it and two Rules entries go with it. The task status is now read explicitly, because `detect-task` probes liveness only on the merge branch; when the tracker cannot be reached both offers stand down rather than guess.
