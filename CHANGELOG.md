...
### Added
- **Rich statusline — experimental, opt-in (lets-ds6bc).** A boxed multi-line statusline for `lets statusline`, behind `--rich` (or `LETS_STATUSLINE_LEVEL=rich`); the default 2-line compact output is unchanged and frozen byte-for-byte. Enable per project by setting the statusline command in `.claude/settings.json` to `lets statusline --rich` (`--light` for a light terminal). Two width tiers driven by `COLUMNS`: **Full** (≥72) and **Compact** (<72), each wrapped in a closed box (`┌─┐ │ ├─┤ └─┘`) that hugs the widest line and is cell-accurate so the right border aligns despite double-width emoji. Lines: identity (growth-stage brand emoji that advances 🌱→🪴→🌿→🌳→🌴 with session lines-added, folder/worktree badge, branch, diff, PR), budget (model + color-graded effort + `window % (used/total) · 5h % (Δ) · 7d % (Δ)`), task (id + title, plus note-count/age/`→ /lets:note` in Full), and a rotating workflow tip. The task line reads a cheap `.lets/cache/task-status` cache that the binary self-refreshes in a detached `bd show` subprocess (90s TTL) — no bd/network on the render hot path. Resilient to Claude Code payload quirks: `workspace.git_worktree` sent as a string and `rate_limits.*.resets_at` sent as a number both decode without blanking the bar. Still opt-in; not yet wired into `lets init`.
