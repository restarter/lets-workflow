...
- **Skill tool migration — 17 pre-existing Read+follow sites (lets-5pkvh).** Extended the Skill tool follow-through mechanism (introduced in lets-sc15i) to all pre-existing command-to-skill / command-to-command invocations: `detect-task` ×12 (`note`/`opinion`/`done`/`plan`/`end`/`execute`/`review`/`github-pr` ×2/`check`/`ask` + `skills/commit`), `actor-fetch-personality` ×4 (`opinion`/`review`/`brainstorm`/`ask`), `take-task` delegation in `start.md`. `commands/commit.md` (3-line delegation shim) DELETED — the skill `skills/commit/SKILL.md` is user-invocable by default and its description lists `/lets:commit` as a trigger, so the slash command resolves directly to the skill. Rule 7 in `plugins/lets/rules/lets-rules.md` rewritten (~370 → ~125 words) — drops now-unnecessary path-resolution details since the Skill tool handles namespace resolution. Worked example, CLAUDE.md table row, internal-invocation note, Command Checklist item all switched from "Read tool" to "Skill tool" wording.
