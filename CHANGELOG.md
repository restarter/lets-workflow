...
- **`/lets:status` → orient snapshot; shared `orient` skill (lets-qsgmd).** Replaced the 5-view `/lets:status` dashboard (overview/ready/labels/blocked/full) with one lean, read-only, tracker-universal orientation — `Where you are` / `In flight` / `Next up` (+ an optional `Project` counts block from the tracker's own `stats`) — extracted into a shared internal `orient` skill now also powering `/lets:start` (which no longer delegates to `/lets:status ready`: decoupled, action-first "what do we do?") and `/lets:backlog --fast` (opens with the same snapshot). Built on `detect-task` + neutral verbs so it degrades cleanly across `beads` / `planfix-mcp` / `none`; the status/priority totals + dependency-graph tree move to native `bd stats` / `bd blocked`, and the per-epic label-progress bars + AI insights are retired from `/lets:status` (still reachable via `/lets:backlog`). The fragile jq/pagination aggregation the task was originally scoped to fix is gone from the `/lets:status` path — status's counts now come from the tracker's native stats (the jq path survives in the `stats` verb binding that `/lets:backlog` uses, a residual noted for follow-up). Markdown + skill only, no Go.
