...
- **`/lets:end` redesigned as a unified settlement pass (lets-mic6d).** Replaces the three divergent modes (full / `--fast` / `--pre-compact`) with one settlement core: independent, auto-skipping checks run in dependency order (uncommitted → commit; this-session work → bd progress comment; task may be done → hand off to `/lets:done`; unpushed → push; always a session summary) surfaced through a single consolidated multiSelect, so a tidy session ends with zero prompts. Adds the previously-missing upstream-aware unpushed-commits check; de-nags the task-status prompt (only offered when there are commits this session); drops the embedded commit-list from both the bd progress and session-summary templates in favour of a `session: <sha>..HEAD (N commits)` range pointer; removes the terminal wrap-up card for a one-line compact/clear prose hint. `--pre-compact` now runs the settlement pass (minus the task-done offer and the terminal hint) plus the always-written resume snapshot, instead of being snapshot-only. `--fast` is deprecated to a no-op alias of the default flow (removed next release). Built on the dsdmp `.task-<slug>` `session:` boundary: `/lets:end` is a pure reader of it and never writes the boundary (`take-task` + the SessionStart hook own writes).
