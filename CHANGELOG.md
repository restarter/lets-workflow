...
### Added
- **Pluggable tracker platform (lets-5d48z).** `LETS_TRACKER` now selects a tracker **adapter** (`beads` \| `planfix-mcp` \| `none`) instead of being schema-reserved; `lets init` installs exactly one drift-tracked `.claude/rules/tracker-<name>.md` binding the neutral verb set (`create`/`show`/`comment-add`/`set-status`/`close` CORE + optional reads) to a concrete transport. beads is the reference adapter — runtime byte-for-byte unchanged, since the `bd` calls in commands ARE its binding, resolved via a single global "Tracker Adapters" rule (no ~180 call-sites rewritten); planfix-mcp is a pure-MCP adapter over the server's generic `planfix_request` REST passthrough (token server-owned, zero new I/O); none is a null adapter. Two files per adapter: the managed `tracker-<name>.md` + a scaffold-once, user-owned `tracker-<name>.board.md` (project status-id map / transitions, never overwritten). `lets init --tracker=<name>` picks on a fresh init (an existing `.env` value wins on re-init, with a warning); `lets update` re-syncs the adapter as an additive `tracker-rules` artifact. Adding a tracker is a one-file recipe (CONTRIBUTING "Add a tracker adapter"), enforced by a contract test (pinned capability table + CORE rows `supported = yes` + `## Degradation`). The highest-density tracker call-sites carry an inline `# tracker: <verb>` anchor so resolution survives context pressure under AUTO MODE. See [docs/trackers.md](docs/trackers.md).
