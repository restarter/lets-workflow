...
- **Bank a session snapshot without ending anything, and stop asserting why (lets-ouy1l, lets-yprsv, lets-tgqlg).** `--pre-compact` was the only snapshot-only path, so recording the state of a long session meant writing a file that says it was made before a `/compact`. The file is permanent and `/lets:start` reads it back, so that was a small lie in a durable record. `/lets:end --session` and `/lets:note --session` write that snapshot with no settlement - no commit, push, progress or finish offers - and the session continues.
  ...
  It is ONE path with four spellings: `--session`, `--snapshot`, `--pre-compact` and `--compact` all do the same thing. An earlier revision of this work shipped `--session` as a second flag beside `--pre-compact`, on the theory that they produced different artifacts. They did not, once both carried a range - the remaining difference was a filename suffix no reader distinguishes and one line of text. The fix for a record that misstates its own purpose is to write that line as an observed fact ("snapshot written mid-session; the session continued past this point"), which is true whether or not a `/compact` follows - not to hand the caller a choice of intentions to assert. Snapshots already written under the old `-snapshot-precompact` name stay readable: every reader of the trail globs wider than the suffix.

  The snapshot carries a `### Range` block whose number is qualified by its trust level, because the boundary reader moved out of `/lets:end` into a new internal `session-boundary` skill - `end` needs it early to gate its own offers, `session-snapshot` needs it late for the block it writes, and two copies of the ladder that fixed lets-370mx would drift apart. `/lets:execute` was reading the same boundary a third time with no validation at all, and now resolves it through the skill like everyone else.

### Changed
- **`/lets:end` refers an open task to `/lets:done` instead of finishing it (lets-mqye8).** `end` opened by declaring it was not task completion and then implemented a slice of it: an option asserting "work looks complete" inferred from a commit count, and a paragraph enumerating what `done` supersedes - `end` modelling `done`'s internals, which goes stale the moment `done` changes and which nothing catches. The offer now rests on a status fact plus observable activity: the task is open, and this session committed, dirtied or got ahead. Whether the work is actually finished stays with `done`'s own Scope Verification, which reads the task description rather than counting commits. The hand-off collapses to one rule, refer and stop, and the supersession matrix and two Rules entries go with it. The gate is `open`/`in_progress` rather than the complement of `closed`, because `in_review` is the status `/lets:done` itself sets after opening a PR and `blocked` is not a state it can advance - referring either would send the user in a circle. The task status is now read explicitly, because `detect-task` probes liveness only on the merge branch; when the tracker cannot be reached both offers stand down rather than guess.
...
- **`/lets:end` no longer reports a session range it cannot stand behind (lets-370mx).** A branch with no `.task-<slug>` fell through to the NON-slugged `.session-start-ref` - one global file answering a per-branch question - and adopted whatever session last wrote it, on whatever branch: a boundary from six months earlier turned a 6-commit session into a confident "260 commits", silently, on a path where "unknown" was already handled safely. That fallback is gone, and every boundary is now validated before use: resolvable, an ancestor of HEAD, at or after this branch's point off the merge branch, and younger than any single session. What survives carries a trust level (`exact` / `prior-session` / `estimate` / `none`) that rides along in the range description into the chat, the snapshot's `### Range` block and the tracker comment, so a number never appears without the qualifier that earned it. When nothing is recorded, a feature branch falls back to its own branch point - branch-scoped by construction - and the merge branch stays unknown rather than guessing. A commit count is never claimed as this session's work on an estimate.
- **`/lets:done` reads the PR back after creating it (lets-ufiam).** A URL from `gh pr create` proves the PR exists, not that it can be built: a PR born with conflicts gets no merge ref, `pull_request` workflows run against that ref, so not one check ever starts and `gh pr checks` reports none - indistinguishable from a repo with no CI. One such PR survived a full `/lets:done` and two further pushes before anyone noticed. The GitHub path now reads `mergeable` and `mergeStateStatus` back, retries the asynchronous `UNKNOWN` a couple of times and reports a persistent one as unverified rather than fine, and on a conflicting PR says the load-bearing part out loud: CI will not run at all until the conflicts are resolved. A clean PR still costs one line; nothing new is gated.
- **`/lets:check` and `/lets:review` are described by who reviews, not by a stopwatch (lets-mbnk8).** The documented "~30 sec" and "~2-3 min" were wrong by roughly an order of magnitude and load-bearing in a way durations rarely are: they set the expectation of whether a command is interruptible, shipped inside a rotating statusline tip, and were quoted in this project's own argument about whether `/lets:check` may ask a question. Correcting the figures would only go stale again, so both commands now say who does the looking - the orchestrator alone, or a selected band of expert subagents followed by an adversarial verify pass - the same axis `/lets:plan` and `--fast` already use, across the rules file, both command bodies, the docs, the README and the statusline tip. `/lets:check`'s spec-reuse rationale is re-grounded on reliability (a remote tracker call can fail and blank the spec), which was the real reason all along.
...
### Added
- **Commands finally consult the adapter (lets-x1rnx).** The 0.7.0 platform gave every tracker a capability table and no command read it, so on a non-beads project half of what LETS did was quietly dead. Adapters now declare what `create`/`set-field` accept and what `show` returns, in neutral field names — `create-task` asks only for fields the tracker can actually store instead of collecting a type and a priority and dropping them without a word. `close` returns the status the task ended in, so a board whose terminal is process-gated (close is not a legal edge from "in progress") gets an advance-and-handoff reported honestly rather than a phantom "closed"; and `/lets:done` now moves the task to `in_review` on opening a PR, where the adapter's own status list says that status exists — before, both PR paths left the task sitting in `in_progress` forever. `detect-task` stops taking the first in-progress task the board offers: an id from the state file or the branch name is this branch's by construction, an id from the board is a guess and is now searched-for and confirmed, never used silently — on a shared board the old behaviour attached to, and commented on, whoever had started something most recently. `/lets:backlog` was the last command sending subagents to run `bd` themselves; its orchestrator gathers the backlog and injects it, on the standard **and** `--workflow` paths (the workflow asset carried its own copy of the prompt, which no test could see). Both beads carve-outs are gone as a concept: the CI exemption list ships empty and is asserted empty, the guard scans every fence and the workflow assets rather than bash blocks only, argument *values* are pinned for the first time, and a `testdata/tracker-fake.md` partial adapter covers the absent-verb / renamed-field / gated-terminal combination that neither shipped adapter has. The `bd` invocations themselves are unchanged and still golden-pinned, but two things beads users will notice: `detect-task` asks instead of auto-picking when a branch carries no task id, and `/lets:backlog` now hands its agents task titles and priorities rather than per-task `bd show` / `bd comments` output - shallower detail, bought back as context the off-context fan-out was there to protect.
- **Bitbucket PR review + finish (lets-6pxvm).** `/lets:review`, `/lets:check` and `/lets:done` now handle Bitbucket PRs via the `bbb` CLI, beside the existing GitHub `gh` path - the forge is resolved from the PR URL host (or `LETS_PR_FLOW`). Review/check fetch metadata, diff and discussion through `bbb` (Bitbucket has no separate review-envelope object, so its discussion is two sources, not three); a same-repo Bitbucket PR gets a real detached branch checkout via git plumbing (fork PRs degrade to diff-review with a message), and the summary posts through `bbb pr comment`. `/lets:done` on `LETS_PR_FLOW=bitbucket` now pushes + opens a Bitbucket PR and leaves the task open for the reviewer to merge - replacing the destructive local-merge that buried the PR and bypassed team review; the former `!= github` local-merge path is narrowed to `== local` (still the fallback for any unrecognized value). The `/lets:github-pr` posting lifecycle stays GitHub-only, and the board-gated `done_action` transition half stays in lets-x1rnx. Unblocks the Bitbucket path of the PR watch-loop (lets-lhnob).
- **tmux worktree launcher + launcher-neutral `lets notify` (lets-0np5i).** `LETS_LAUNCHER=tmux` (or `/lets:worktree create --tmux`) is the portable third launcher — cross-platform on unix (Linux + macOS), no GUI dependency, the option Linux users get since cmux is macOS-only. New `lets tmux open/rename/notify` Go subcommand (`cli/internal/tmuxcmd/`, `//go:build unix`; Windows stub) mirrors the shipped cmux launcher file-for-file (same JSON envelope, typed exit codes, never-hard-fail contract), driving the real `tmux` CLI: `open` opens a new window inside `$TMUX` or a detached session outside it (`send-keys` delivery, **never auto-attaches** — surfaces `attach_command`), with a `pane_current_path` duplicate guard. Gate notifications now route through a new launcher-neutral **`lets notify`** (`cli/internal/notifycmd/`) that dispatches on `LETS_LAUNCHER` — the `plan-workflow.md` / `execute.md` gate snippets no longer hardcode `lets cmux notify`, so a tmux operator finally receives them. Two tmux-specific correctness fixes, both verified against live tmux 3.6b: `notify` broadcasts to every attached client and reports `no_client` (not a phantom `Notified=true`) when the worktree's session is detached — the pipeline's own default; and `--description` is stamped into the `@lets_task` window option rather than the width-budgeted window name (the tmux window name is the status line). `LETS_LAUNCHER` is now validated against `letsconfig.ShippedLaunchers` (`lets init --launcher=<unknown>` is rejected), pinned to the registered subcommands by a test — the launcher analogue of the tracker platform's on-disk pin. The `/lets:init` launcher question is detection-aware (probes `uname` + `command -v`; never offers cmux on Linux; marks a launcher `(detected)` when its binary is on PATH). Not to be confused with the existing `make dev-tmux` dev harness. Docs synced across CLAUDE.md, cli/README.md, README.md, docs/autonomous.md, docs/configuration.md, and the session-hook config explainer.
- **Reviews read the task spec (lets-yobxe).** `/lets:review` and `/lets:check` resolve the active task's description through the tracker's neutral `show` verb and inject it into the reviewer prompts. Without it a reviewer sees code nothing calls yet and confidently reports it as dead - the observed failure was a `[BLOCKER]`-severity false positive on work whose wiring was a later PR. Work the spec describes is planned work, not scope creep; with no spec the review still runs and says so on any scope finding, without downgrading it - a missing spec is missing information about intent, not evidence that the code is fine, so reviews on a project with no task tracker are exactly as sharp as any other. `--file` gets the spec too, which turns a file review against acceptance criteria into a completeness check. The skeptic's copy of the block is deliberately narrower: its only lever is `real`, so any reviewer-style "do not report this" would come out as `real=false`, which the drop rule turns into a silent delete.
- **`/lets:review <PR>` offers to check out the PR branch (lets-yobxe).** Previously PR mode piped the diff into the prompt while the working tree stayed on the base branch, so `Read`/`Grep` and the adversarial verification pass read the wrong files. It now asks once; "Review from diff" changes nothing on disk and tells the reviewers the files they see are the base branch. It never creates a worktree - where you review is your call - and your branch is restored afterwards. The checkout is detached, so a review that never finishes restoring cannot leave a branch name for LETS's per-branch task state to latch onto; if a session dies mid-review, the next `/lets:review` and the next `/lets:start` both surface the leftover instead of stranding it. Project rules (`CLAUDE.md`) are read from the merge branch even after the switch - a PR's own edits to them belong in the diff a reviewer judges, not in the prompt slot every agent obeys.
- **`detect-task` accepts `branch=<ref>` and `fallback=no` (lets-yobxe).** Both opt-in and off by default, so existing callers are unchanged. `branch=` lets a command resolve a task for a ref other than the checked-out one, with the untrusted-ref sanitization living in the skill that owns ref→id derivation rather than copied into callers. `fallback=no` skips the `list-by-status` step, which answers "some task is in progress" rather than "this branch's task" - on a shared board that is a colleague's. `/lets:review` and `/lets:check` pass it when resolving the spec, so an ambiguous board can no longer put the wrong task description in front of the reviewers.
- **You say where the spec is (lets-2z0hw).** `/lets:review` asks once - in the same prompt as the PR-branch question, so it is one interruption - and the options name what it actually found: your task by id and title, the newest plan file matching the branch by name, or **no spec**. "Other" takes a task id or any path. `--spec <path>` / `--spec none` / `--spec <task-id>` answers it up front and skips the question, and is how `/lets:check` gets the same control without ever asking: check is fired repeatedly while writing code, review is the considered checkpoint. **"No spec" is a real answer**, not a failure - no spec block, no caveat, because reminding a tracker-less project on every single review is noise; a lookup that *failed* still says so, since something may exist that could not be read. And a missing spec no longer downgrades anything: it is missing information about intent, not evidence that the code is fine, so reviews on a project with no tracker are exactly as sharp as any other. `--file` resolves a spec like every other mode now - with acceptance criteria in hand, a file review becomes a completeness check rather than a taste test.
- **Reviewers get the PR's description and its whole discussion (lets-2z0hw).** Three different things reach a reviewer and only one is the spec: what was *supposed* to be built, what the author *claims* they built, and what people have *already said*. The description used to arrive only when no spec could be found, and the discussion never arrived at all - so a review would re-report a finding that a thread had already raised, fixed and verified on that very line. Gathering it takes three separate calls on GitHub and the obvious one returns barely half: measured on this repo's PR #2, 9 texts across issue comments, review bodies and line-anchored inline comments, where `gh pr view --json comments` alone returns 5 and counting the empty review envelopes inflates it to 11. None of it reaches the adversarial verification pass - it is written by the author of the code under judgement or by people commenting on it, and a verifier can only answer "real or not", so one "we agreed to ignore this" would delete a finding outright. For the same reason a PR description is **never** treated as the spec any more: on a PR with no resolvable task there is simply no spec, and the description is read as the claim it is. `--branch` also stops diffing against a local merge-branch, which in a worktree is stale by construction, and resolves the base itself.

### Changed
- **Workflow rules: a fenced block containing prose is prose (lets-ohqzm).** The no-hard-wrapping rule was not being disobeyed so much as classified around - inside a fence the content reads as a block where column formatting looks appropriate, so the rule silently did not apply, and the rule's own closing clause named "code blocks" as a place line breaks belong, which reinforced the misread. The net effect was column-wrapped tracker comments shipping even though the rule was present, because knowing a rule cannot fix a classification bug. The fence is not the classifier, the destination is: text that lands somewhere rendering markdown and soft-wrapping on its own is prose and gets one line per paragraph, even when typed inside a fence or a heredoc; columns inside a fence are for genuinely columnar destinations only - code, argv, an ASCII table or diagram, real terminal output. The closing clause no longer reads as a blanket line-break exemption for every fence.

## [0.7.0] - 2026-07-09

### Added
- **Pluggable tracker platform (lets-5d48z).** `LETS_TRACKER` now selects a tracker **adapter** (`beads` \| `none`; other trackers are user-authored, not shipped) instead of being schema-reserved; `lets init` installs exactly one drift-tracked `.claude/rules/tracker-<name>.md` binding the neutral verb set (`create`/`show`/`comment-add`/`set-status`/`close` CORE + optional reads) to a concrete transport. beads is the reference adapter — command/skill bodies speak neutral ` ```lets-tracker ` verb blocks, and beads resolves each through `tracker-beads.md` (golden-pinned to the exact `bd` invocations LETS always ran, so beads runtime is unchanged); none is a null adapter (planfix-mcp was prototyped over the server's generic `planfix_request` REST passthrough but is NOT shipped — it's a user-authored adapter example, see docs/trackers.md). One managed `tracker-<name>.md` per adapter, plus an optional scaffold-once, user-owned `tracker-<name>.board.md` (project status-id map / transitions, never overwritten). `lets init --tracker=<name>` picks on a fresh init (an existing `.env` value wins on re-init, with a warning); `lets update` re-syncs the adapter as an additive `tracker-rules` artifact. Adding a tracker is a one-file recipe (CONTRIBUTING "Add a tracker adapter"), enforced by a contract test (pinned capability table + CORE rows `supported = yes` + `## Degradation`) plus a golden pin on the beads bindings and a durable test that no executable `bd` survives in a command body. Every executable `bd` verb-call across ~25 command/skill files was rewritten to a neutral ` ```lets-tracker ` block; the lone allowlisted exception is the detect-task merge-branch liveness probe, a beads-only read gated on the active tracker. A shared `tracker-<name>.md` is trusted instruction auto-loaded into model context — review its bindings before installing one you didn't author (the contract test pins table shape, not binding safety; a token belongs only in the transport's own config, never in a loaded/shared file). See [docs/trackers.md](docs/trackers.md).
...
### Changed
- **Agent model policy: drop frontmatter model pins, inherit the session model (lets-f1lu1).** The per-agent `model: opus` pins (`architect`, `backend`, `security`, `actor`, `implementer`) were a stale UPWARD pin that *downgraded* those agents on a Fable session (Opus 4.8 < the session model) — pinning "the strongest" by name ages the moment a newer model ships. All five dropped; agents now inherit the session model (newest = strongest). `explorer` keeps its `sonnet` cost-pin (mechanical codebase-mapping — a downward pin can't cause a capability downgrade); verify (`skeptic`) deliberately stays on the session model, since a cheap verifier false-drops hard `[BLOCKER]`s (the drop rule refutes a BLOCKER when all skeptics vote `real=false`, and a weaker model leans that way when it can't substantiate a deep finding). No config knobs. Supersedes lets-oy3ee. Docs synced (CLAUDE.md + review/opinion/ask).
- **`/lets:status` → orient snapshot; shared `orient` skill (lets-qsgmd).** Replaced the 5-view `/lets:status` dashboard (overview/ready/labels/blocked/full) with one lean, read-only, tracker-universal orientation — `Where you are` / `In flight` / `Next up` (+ an optional `Project` counts block from the tracker's own `stats`) — extracted into a shared internal `orient` skill now also powering `/lets:start` (which no longer delegates to `/lets:status ready`: decoupled, action-first "what do we do?") and `/lets:backlog --fast` (opens with the same snapshot). Built on `detect-task` + neutral verbs so it degrades cleanly across `beads` / `none` / any custom adapter; the status/priority totals + dependency-graph tree move to native `bd stats` / `bd blocked`, and the per-epic label-progress bars + AI insights are retired from `/lets:status` (still reachable via `/lets:backlog`). The fragile jq/pagination aggregation the task was originally scoped to fix is gone from the `/lets:status` path — status's counts now come from the tracker's native stats (the jq path survives in the `stats` verb binding that `/lets:backlog` uses, a residual noted for follow-up). Markdown + skill only, no Go.
- **LETS box → Response Footer model (lets-bh79a).** The bottom-of-response affordance is now one governed model instead of ad-hoc per-command ASCII: every response ends with exactly ONE footer of a known type — **Act** (AskUserQuestion, the AI executes the pick) / **Nav** (LETS box, the user types a `/lets:*`) / **Close** (a prose line or nothing) — never mixed. Nav content is **state-driven**: a box never offers `/lets:start` while a task is active (it re-runs task selection) — that was a real bug in the post-plan and post-review boxes, now fixed; the "clean context" intent is a `/clear` + mid-task-command prose hint that keeps the task. The `lets-rules.md` governance is rewritten from "every milestone shows a box" (which contradicted "no command → no box") into a lean contract (Act/Nav/Close + a phase→type→content table + one box-format example). `/lets:start`'s normal-flow box drops the `Working on:` info into prose (a box is navigation, not status); `/lets:statusline` drops its malformed box for a prose hint (no `/lets:*` next step). CONTRIBUTING's box spec + width-verify recipe are corrected (width is measured in display columns / code points, not bytes — the box glyphs are 3 bytes each). Markdown-only.
- **`/lets:end` redesigned as a unified settlement pass (lets-mic6d).** Replaces the three divergent modes (full / `--fast` / `--pre-compact`) with one settlement core: independent, auto-skipping checks run in dependency order (uncommitted → commit; this-session work → bd progress comment; task may be done → hand off to `/lets:done`; unpushed → push; always a session snapshot) surfaced through a single consolidated multiSelect, so a tidy session ends with zero prompts. Adds the previously-missing upstream-aware unpushed-commits check; de-nags the task-status prompt (only offered when there are commits this session); drops the embedded commit-list from both the bd progress and session-snapshot templates in favour of a `session: <sha>..HEAD (N commits)` range pointer; removes the terminal wrap-up card for a one-line compact/clear prose hint. `--fast` is deprecated to a no-op alias of the default flow (removed next release). Built on the dsdmp `.task-<slug>` `session:` boundary: `/lets:end` is a pure reader of it and never writes the boundary (`take-task` + the SessionStart hook own writes).
- **Session-context capture unified into one file-primary `session-snapshot` primitive (lets-ade5b).** The internal `pre-compact-note` skill is renamed to `session-snapshot` and inverted to **file-primary**: it ALWAYS writes the recovery-grade `## RESUME` snapshot to a `.lets/sessions/` file (the trail `/lets:start` reads), adding a one-line pointer to the active task only when one is unambiguously active (never a `list-by-status | head -1` guess). `/lets:end` (default), `/lets:end --pre-compact`, and `/lets:note --pre-compact` all delegate to it, so the template and file/pointer behavior can't drift. `--pre-compact` is now **snapshot-only again** (NO settlement) and byte-identical to `/lets:note --pre-compact` — both early-exit / delegate with the skill owning task detection (no ambiguity prompt). Fixes the same-minute overwrite where the summary file and the RESUME fallback both wrote `<ts>-precompact-<slug>.md`. `/lets:start`'s quick `<task-id>` resume path (via `take-task`) now reads the latest snapshot file, and a post-compact workflow rule reminds the model to reconstruct from in-window context AND still read the snapshot file (the file wins on conflict). Supersedes the `--pre-compact`-runs-settlement behavior from the unreleased lets-mic6d. Also renames the `/lets:end` / `/lets:note` `--pre-compact` alias from `--resume` to the shorter `--compact` (a spec-only alias since 0.6.0, absent from user docs; `--compact` names the `/compact` you're about to run).
...
- **Workflow rules: drop the 256k-era "long session → `/lets:end`" nag (lets-6jo3i).** The rules no longer tell the model to push `/lets:end` / a "fresh window" based on a guess about session length (tool-call count, files touched, "hours of work") — it can't measure context pressure, and the guess contradicted the sibling "use `/context`, don't speculate" rule. Wrapping up is now raised only on a real signal (the user brings it up, compaction is imminent, or `/context` shows it's high). The surviving kernel is reframed as branch-focus hygiene (finish or park the current task before a new large scope), independent of session length.
