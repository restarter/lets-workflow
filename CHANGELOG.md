...
### Added
- **Rich statusline is now the default (lets-ds6bc).** `lets statusline` renders a boxed multi-line statusline; `--compact` falls back to the legacy 2-line output (kept for terminals where the box misbehaves). Flags: `--light` (default dark), `--no-tip` (or env `LETS_STATUSLINE_TIP=off`) hides the bottom tip line; `--rich` is a hidden accepted no-op. Two `COLUMNS` tiers — **Full** (≥72) and **Compact** (<72), each in a closed box (`┌─┐ │ ├─┤ └─┘`) that is cell-accurate so the right border aligns despite double-width emoji; below 90 cols the box fills the window, above it hugs the content, with a small right gutter so it never overflows Claude Code's render area. Lines: identity (growth-stage brand emoji 🌱→🪴→🌿→🌳→🌴 by session lines-added, folder/worktree badge, branch, diff, PR), budget (model + color-graded effort + `window % (used/total) · 5h % (Δ) · 7d % (Δ)`), task (id + title that clips first, keeping note-count/age/`→ /lets:note`), and a rotating tip. The task line reads a `.lets/cache/task-status` cache the binary self-refreshes in a detached `bd show` subprocess (90s TTL; control bytes stripped from titles). Resilient to Claude Code payload quirks: `workspace.git_worktree` sent as a string and `rate_limits.*.resets_at` sent as a number both decode without blanking the bar.
