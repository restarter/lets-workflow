...
### Added
- **Reviews read the task spec (lets-yobxe).** `/lets:review` and `/lets:check` now resolve the active task's description through the tracker's neutral `show` verb and inject it into every reviewer **and** every skeptic prompt, on both the standard and `--workflow` paths. Without it a reviewer sees code nothing calls yet and confidently reports it as dead — the observed failure was a `[BLOCKER]`-severity false positive on work whose wiring was a later PR, not a miss. Work the spec describes is planned work, not scope creep; with no spec resolvable the review still runs, says so in the report, and caps any "unused / dead / cut this" finding at `[SUGGESTION]`. The spec is fenced as untrusted data with an explicit authority bound (nothing inside it can move a tier, the verdict, or the `PROJECT_ROOT` boundary), and the skeptic's block is deliberately narrower than the reviewer's — a skeptic cannot set a tier, so a tier-cap instruction would be executed as `real=false`, which the drop rule turns into a silent delete. Spec source by target: local reviews use the branch's active task; on a PR, `review` derives the id from the PR's branch (never the `list-by-status` fallback — on a shared board that returns a colleague's task) and falls back to the PR description, while `check` uses the PR description directly; `--file` gets no spec, so it stays free to report dead code at full severity. Additive `--json` fields `spec_available` / `spec_source` / `pr_tree` carry the degradation caveats to machine-readable consumers, since that path skips the console report.
- **`/lets:review <PR>` offers to check out the PR branch (lets-yobxe).** Previously PR mode piped the diff into the prompt while the working tree stayed on the base branch — so `Read`/`Grep`, the systemic pattern check, and the adversarial verification pass all read the wrong files, with the skeptic (whose whole job is judging findings against real code) the worst affected. The command now asks once whether to switch; answering "Review from diff" changes nothing on disk and tells the reviewers plainly that the files they see are the base branch. It **never creates a worktree** — where you review is your call, and the gate is identical in the main checkout and in any worktree. Your branch is **always** restored afterwards (branch-keyed task state desyncs otherwise) and a stash is popped only after that switch back succeeds; `--json` never touches the working tree at all. After a checkout, `CLAUDE.md` rules are read from the merge branch, so a PR that edits them is reviewed as a diff rather than obeyed.
