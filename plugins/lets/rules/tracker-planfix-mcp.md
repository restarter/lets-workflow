...
- **String separators do NOT work.** `value:"114;113"` or `"114,113"` are silently mis-parsed (status filter -> status 0, the account's Draft status; task-number filter -> empty). A SINGLE value may be a string (`"114"`) or int; a SERIES must be a real array. (This was misread as "filters broken" twice in testing - it was the wrong value type.)
