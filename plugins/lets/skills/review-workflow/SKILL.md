...
| `prContext` | string | the PR's own description plus its full discussion (issue comments + non-empty review bodies + inline threads), **empty outside PR mode**. Drives `prContextBlock` in the review prompt and **nothing else**: it has no skeptic counterpart by design, since every word is written by the author of the code under judgement or by people commenting on it, and a skeptic's `real=false` is a deterministic drop. Sanitized by the same `fenced()` as `spec` (400 lines / 20000 chars; both fence names neutralized in both values) |
