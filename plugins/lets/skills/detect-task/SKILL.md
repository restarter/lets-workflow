...
**`--orc` is stripped first.** Before the id-shape test, remove an orchestrator binding from the argument - `--orc=<name>`, `--orc="<name with spaces>"` or `--orc <name>` - and test only the remainder as the id (so `/lets:plan-workflow lets-abc --orc="MAIN PWA"` still claims `lets-abc` instead of becoming a free-text goal). Never validate the name in markdown: forward it raw and let Go refuse it.
- Claim needed -> `Skill(skill: "lets:take-task", args: "<id> orc=\"<name>\"")`; take-task records the binding with the claim.
- Task already `in_progress` (take-task is skipped) -> record the binding yourself, single-quoting the name (`'\''` for a quote inside it): `lets worktree task-state set --create --orc '<name>' --json`. No `--task`: the file may hold another task's `task:` / `start:`, and a binding belongs to the branch. `written=false` (e.g. `orc_on_merge_branch`) or a refused name -> one line, nothing else changes.
