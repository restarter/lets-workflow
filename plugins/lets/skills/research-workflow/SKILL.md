...
- **Consume `parallel(...)` output via `.filter(Boolean)`** (both phases) - the house defensive idiom (review/explore/backlog/opinion all do it). A slot can be null if a thunk throws synchronously; never destructure a null after the expensive fan-out.
