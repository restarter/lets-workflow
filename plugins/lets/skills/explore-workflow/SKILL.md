...
| `profile` | string | the scout's Topic Context Profile (in-context Phase 1 output, ~500 words) |
...
- Top-level `await`/`return` are used, so the file is NOT Node-importable - no committed unit test; the web-research stage, the cluster agent + `buildThemes` / `clusterIdeas` / `formatWebBrief` are kept in sync with `explore.md` (Web Research / Phase 4 / W4) prose by discipline + validated by the live smoke test (the pure logic - `buildThemes`, `clusterIdeas`, `formatWebBrief` - is deterministically checkable on a copy / via a stubbed-hook harness).
