...
| `/lets:handoff` | Code | Hand-off brief for another agent - same targets as `/lets:review`, plus `--commits` / `--range`; `--codex` / `--send` deliver it and bring the report back verified (old name `/lets:review-handoff` still works) |
