...
`fallback=no` in BOTH modes. The fallback now asks before it answers, and a review fan-out is the last place that can stop for a question - while the id it would resolve feeds every reviewer prompt AND every skeptic prompt, where a wrong spec is worse than none. It also makes the result unambiguous *by construction*, which is what a plain call cannot give: an id came from the state file or the ref name, never from the board.
...
Systemic findings and `[NIT]`s never enter the table: Step 6.5 only measures how far a pattern spreads, it does not verify the defect, and a systemic finding spans files outside the scope. Then `Skill(skill: "lets:apply-fixes", args: "source=review mode=<local|staged|last-commit|branch|pr|file> base=<BASE> path=<path>")` - `base` is the Step 2 `BASE` for `--branch`; in PR mode list the changed files of the PR diff Step 2 fetched, taken from that diff, in the conversation (`apply-fixes` PR scope) - never a new fetch, never a local merge-base; `path` for `--file`. Its output replaces the Output box.
...
if [ -z "$SLUG" ]; then
  ...
  # an idea document only when no plan exists - reviewers are told it is one (P3)
...
Read the plan file and show title + task ID to user. When it is an idea document (the name matches `-idea` or it carries `IDEA BANK ENTRY`), say so, and inject this one line into every reviewer prompt: "This is an idea document; judge feasibility, clarity, scope and open questions, not implementation steps."
