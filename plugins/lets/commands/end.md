...
2. **Finish task** -> **RECORD, then REFER + STOP.** First run **Step 3a only** with `pointer=off` - the session file, no task pointer. Right here the branch, HEAD and the task are still this session's, which is the state the snapshot exists to capture; the stop below is about writing AFTER the hand-off, and loses none of its force by letting the record land before it. Skipping it cost a real session its record (lets-fkm0n), on the longest kind of session there is. Step 3b does NOT run: it is a task-side record and `done` writes its own. Then say "Handing off to /lets:done" and name any other picks being dropped with it (`multiSelect` lets the user select alongside it, and silently discarding a pick is worse than declining it). Then `Skill(skill: "lets:done")` and **STOP the entire /lets:end flow** (do NOT run Step 3b, Step 4, or Output). That is the whole rule, and it is deliberately the whole rule: `end` does not enumerate what `done` will redo, because once `end` stops there is nothing left for it to reason about. `done` owns the task lifecycle from here - its own scope check, its commit/push/PR/close, and its terminal menu, which may switch branches or call `/lets:end` again. That re-entry is safe BECAUSE S3 gates on `open`/`in_progress`: `done` leaves the task `closed` or `in_review`, neither of which re-arms this offer. Stopping is what keeps any FURTHER branch-slugged write off a branch `done` may have moved away from - the one that mattered was already written, above, while the branch was still ours.
...
Write the session-level snapshot that bootstraps the next `/lets:start`, via the shared primitive so every snapshot kind shares one template and none of them drift. The `pointer` arg depends on which record the task is going to receive from elsewhere:
...
- `pointer=off` (**Finish-task referral**, called from Step 2 before the hand-off) -> same reason, different writer: `/lets:done` puts its own completion comment on the task, so a pointer from `end` would be a second record a minute apart. The file still lands, which is the whole point of running 3a there.
