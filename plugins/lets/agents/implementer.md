...
When your brief or your latest AMENDMENT carries `REPORT_FILE: <absolute path>`, write the report to that path in ONE Write call, with `REPORT-END` as the file's last line, then send a final message of exactly two lines: `REPORT_WRITTEN <path>` and the report's `**Status:**` line. Every round writes the REPORT_FILE its latest brief or amendment names; that file is the only file outside YOU MAY WRITE ONLY you ever write. No REPORT_FILE -> send the report itself as your final message.
