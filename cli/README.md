...
- **`terminal`** (lets-0rgnd, merged; ships next release) runs `terminal create --worktree path:<w> --title <t> --command <c>` in an EXISTING worktree LETS created - a standing team's lead - and returns its `handle`. It never creates an Orca worktree (there is no `lets orca create`; `open` is unchanged). Bad usage is the only hard error (a relative or missing worktree, an empty title, a newline or control character in the title or command); Orca absent, not running or refusing gives `launched=false`, a `reason` and a `fallback_command` (`cd <worktree> && <command>`) to print.
