...
- **Notifications — deferred (cmux is native).** cmux already surfaces notifications out of the box (`cmux notify --title/--subtitle/--body --workspace`, plus `list-notifications` / `dismiss-notification` / sidebar), so no `lets cmux notify` wrapper is built. This package stays the planned home for a `lets cmux notify` should a LETS-specific event ever need surfacing that cmux does not already show.
