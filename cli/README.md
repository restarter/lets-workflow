...
lets worktree pushed --commit <sha> [--branch <b>] [--timeout <dur>] [--json]
lets worktree team-init (--suggest-callsign | --check --callsign <c> | --callsign <c> --area <a> --worktree <abs>) [--plugin-root <dir>] [--agent-command <cmd>] [--orca-agent <cmd>] [--json]
lets worktree switch --task <id> [--branch <b>] [--title-file <file>] [--park [--include <path>]...] [--fetch-timeout <dur>] [--json]
lets worktree parked --team <callsign> [--json]
```
...
- **`pushed`** (merged; ships next release) answers whether `--commit` is on ANY branch of ANY configured remote: `state` is `pushed` (with `remote` / `head`), `not_pushed`, or `unverified` with a `reason` (`too_many_heads`, `git_error`). Every state exits 0; only `not_pushed` permits a history rewrite - `/lets:execute --pipelined`'s closing autosquash asks it first.
- **`team-init`** (lets-0rgnd, merged; ships next release) writes a standing team's file `.lets/teams/<callsign>.md` from `<plugin-root>/templates/team.md`. `--suggest-callsign` proposes the first free callsign of `teamfile.Callsigns` (40 animals; free = no team file and no live session named `<callsign>-lead`) and `--check` verifies a given one - both write nothing, and `/lets:worktree create --team` runs them BEFORE the team worktree is created. The file is rendered to a fsynced temp in `.lets/teams/` and published with `os.Link`, which the filesystem refuses when the target exists - never overwritten by any writer, and two racing inits get exactly one winner; a filesystem that refuses links fails with nothing written. Frontmatter: team, area, repo, worktree, git_dir, base, created, lead_name `<callsign>-lead`, agent_command and orca_agent (both default `claude`).
- **`switch`** (lets-0rgnd, merged; ships next release) moves a standing team's worktree to another task's branch. It refuses before touching anything: 29 `not_team_worktree` (no team file claims it), exit 14 kinds `index_unmerged`, `operation_in_progress` (a merge / cherry-pick / revert / rebase marker) and `detached_head`, 32 `members_live` (only a live, rotated or unknown in-tree `lets:implementer` of any members registry - the read-only team members and an isolated implementer never block; an unreadable registry refuses), 33 `target_is_merge_branch`, 14 `dirty_worktree` without `--park`, 31 `no_remote_base` for a new branch with no `origin/<merge>`, `session_held` (1) when the target's `session:` line is held by another session (the SessionStart guard's rule - switch from the team's lead session). A new branch is cut from `origin/<merge>` (fetched, 20 s; a stale ref is used with `base_stale`) with `--no-track`, never from the local merge-branch. `--park` commits the old branch's changes as `wip(<id>): park`: every NEW path (untracked, or staged as added / copied / renamed) must be named with `--include`, else 30 `untracked_present` lists `new_paths` with the index and HEAD untouched; ignored files are never parked - they stay on disk in the worktree; nothing here ever stashes. The park records `park:` and `park_team:` in the old branch's task-state. Arriving on a parked branch undoes the park (`reset --soft HEAD~1`, changes staged again) only when the park commit is HEAD, its subject matches and no remote has it (`unpark`: `unparked` | `parked_pushed` | `parked_unverified` | `parked_kept`). The target's task-state gets `task`, `start`, `orc` carried from the old branch, and `session` through the SessionStart guard.
- **`parked --team <c>`** (lets-0rgnd, merged; ships next release) lists the parks that team made (`owned`: task, branch, park_sha, tip) and parks that name no team (`unowned`); another team's parks are left out. `/lets:team disband` reads it.
- **`info` reports `team`** (merged; ships next release) - the standing team whose team file claims this worktree (`teamfile.FindByWorktree`); a malformed, stale or ambiguous team file is a `warn` step, never a failure.
...
lets orca terminal --worktree <abs-path> --title <name> --command <cmd> [--json]
lets orca team-create --name team_<callsign> --base-branch origin/<merge> [--repo <main>] [--json]
lets orca team-remove --name team_<callsign> [--repo <main>] [--json]
```
...
- **`terminal`** (lets-0rgnd, merged; ships next release) runs `terminal create --worktree path:<w> --title <t> --command <c>` in an EXISTING team worktree - Orca-created on the orca launcher, Go-created otherwise - for a standing team's lead, and returns its `handle`. It never creates a worktree (`open` is unchanged). Bad usage is the only hard error (a relative or missing worktree, an empty title, a newline or control character in the title or command); Orca absent, not running or refusing gives `launched=false`, a `reason` and a `fallback_command` (`cd <worktree> && <command>`) to print.
- **`team-create`** (lets-0rgnd, merged; ships next release) creates a standing team's worktree THROUGH Orca on the orca launcher, so Orca registers it and `terminal` can attach the lead: `worktree create --repo path:<main> --name team_<c> --base-branch origin/<merge> --setup skip --no-parent --json` - no agent, no prompt, the setup script skipped (LETS runs its own gated hook). Names are `team_<callsign>` only, and the base must be `origin/<branch>` (Orca resolves a bare name as the local ref). States: `not_attempted` (Orca absent, not running, or its `worktree ps` failed - the only state a caller may answer with a Go create), `already_exists` (a ps row of that name, compared after the `refs/heads/` trim), `created` (Orca's ps AND git's worktree list both show it, paths compared with `fsutil.SameDir`), then verified: basename, branch `team_<c>`, HEAD == the base; the terminals already open there are listed, and a tracked agent among them is refused. An attempted create never degrades to `not_attempted` and is never retried: exits 11 `create_ambiguous` (the two sides disagree), 12 `create_failed` (neither shows it), 13 `team_worktree_mismatch`, 14 `agent_terminal_present` - the worktree stays in every case.
- **`team-remove`** (lets-0rgnd, merged; ships next release) removes an Orca-created team worktree through `worktree rm --worktree path:<p> --run-hooks --json` - never `--force`, never `--allow-failed-archive-hook`. First its nets, each a separate command in the worktree: no uncommitted or untracked change (exit 15 `dirty_worktree`), HEAD proven on a remote through `gitutil.RemotesContainAny` (`not_pushed` or `unverified` -> exit 16 `unpushed_commits`). Orca unavailable -> `not_attempted`, exit 0 (Go never removes an Orca worktree); no ps row of that name -> exit 19 `not_listed` (a Go-created team worktree, removed by `/lets:worktree remove`). After the rm, Orca's `worktree_archive_hook_failed` -> exit 17 (the worktree stays, the hook output shown); git and ps both without it -> `removed`; anything else -> exit 18 `remove_ambiguous`. Never retried.
- **Exits 11-19** break orcacmd's never-hard-fail convention on purpose: they fire only after a create or remove was attempted, or when a safety net refuses; Orca being unavailable stays exit 0.
...
- **Bound sibling repos (lets-th8ti).** Only for a BOUND name and only with `LETS_LAUNCHER=orca`: when this repo cannot address the bound orchestrator, `orchestrator` looks for it among the main checkouts `orca repo list` returns - the session's live cwd must lie in one of them, and exactly one live orchestrator there may carry the name (the same two passes as in-repo: the live holder of the name, else a role file registered under it; a re-minted id is reconciled in memory). The target comes back with `repo_index`, which the orc skill passes to `tell` / `tail --repo-index`. The other direction: `who --orc <name>` under Orca also lists the workers in sibling repos whose branch binding names that orchestrator, each with `repo_index` and `repo` (the Orca repo's display name) - except in a sibling that has its own live orchestrator of that name. Both read the sibling read-only (lets-rules Boundaries, "Carve-out - bound sibling sessions"); a repo that cannot be read degrades by name, and a spent budget is `budget_exhausted`, never a configuration verdict. `refused[].detail` for `target_in_other_repo`: `orca_not_selected` (reach needs `LETS_LAUNCHER=orca`), `orca_unavailable`, `repo_not_in_orca`; several live holders across siblings are `bound_ambiguous` / `sibling_repos`. Every remediation names only a command that can act on that refusal - `remedy_test.go` pins it, and `/lets:hub` is named only for a stopped orchestrator.
...
(merged; ships next release) The registry of the members a lead spawned in one scope and of the scope's lead (`internal/memberscmd/`, `//go:build unix`; the non-unix stub fails every call). Scope = a standing team's callsign (`teamfile.ValidName`) or an execute run's `run-<RUN>`. Store `.lets/execution/members-<scope>.json`, written through `fsutil.AtomicWriteBytes` under `.lets/locks/members-<scope>.lock` (5 s). The member-run skill is its one caller in markdown; the session id, pid and caller toplevel are computed in Go (`$CLAUDE_CODE_SESSION_ID`, the Claude Code session registry, the git toplevel), never taken from a flag.

```bash
lets members add --scope <s> --name <n> --role <lets:role> [--model <m>] [--isolation worktree --worktree-path <abs> --worktree-branch <b> --agent-id <id>] [--link team|peer] [--cwd <dir>] [--json]
lets members dismiss --scope <s> (--name <n> | --all) [--json]
lets members status --scope <s> [--name <n>] [--json]
lets members lead --scope <s> [--claim] [--json]
```

- **`add`** records a member right after its Agent call. It looks for the member's OWN session - a registry entry named its Agent name (`<callsign>-<name>` in a team scope, the bare name in `run-*`) at `--cwd` (default: `--worktree-path` when isolated, else the toplevel), 5 reads 1 s apart: found -> `kind: pane` with that session's sid / pid; not found -> `kind: in_process`, recorded on the caller's session, whose liveness it then shares. An isolated member needs `--agent-id`: the harness runs it as a background agent reachable only by that id. The role must be a shipped `lets:*` agent other than `actor` (`ShippedRoles`, pinned to `plugins/lets/agents/*.md`). A name that is `live`, `rotated` or `unknown` is refused; a gone or dismissed one is replaced. In a team scope only the recorded lead (or the id it was re-minted into) adds a member.
- **`status`** judges the lead and every member now. `live` = the session id runs under the recorded pid (the lead: under any pid - `claude -r` keeps the id); `rotated` = the same process re-minted the id (/clear), carried to the new id under the lock; `gone` with `session_dead` | `worktree_missing` | `prunable` | `dir_missing` (a locked, isolated worktree whose directory vanished) | `dismissed`; `unknown` when the registry cannot prove either way (never treated as gone). A pane member's `link` flips `team` -> `peer`, and is saved, once the lead that spawned it is replaced. For an execute scope, `live` means "resumable by name while the lead lives" - `op=next` resumes the persistent implementer; a vanished worktree is already `gone`. A scope with no registry file is empty, and nothing is written.
- **`lead`** prints the recorded lead; `--claim` records this session. A dead lead is taken over (the reopen case); `lead_held` while another lead is `live`, `rotated` into another session, or `unknown`. `memberscmd.RefreshLead` (the SessionStart hook) refreshes the lead's pid after `claude -r`; it waits 1 s for the lock and a busy lock is silent.

Exit codes (40-49): 40 `invalid_scope`, 41 `name_invalid` (also: not a member of the scope), 42 `name_live`, 43 `role_not_allowed`, 44 `lead_held`, 45 `no_lead`, 46 `lock_busy`, 47 `registry_unavailable` (no session id, no registry, this session not in it, or `--link peer` without the member's own session), 49 `io_error`; 2 usage. `SchemaVersion` 1, `TestResult_SchemaContract`.

## lets integrate

(merged; ships next release) Lands one chunk of an isolated implementer's branch in the caller's tree as ONE staged patch (`internal/integratecmd/`, `//go:build unix`), for `/lets:execute --parallel`. Never a merge, never `reset --hard`, never a commit - the lead commits.

```bash
lets integrate --from <commit-ish> --since <commit> --run <run> --chunk <chunk> [--json]
lets integrate --revert --patch <patch_path> [--json]
```

- **Integrate.** Refuses a dirty caller tree (untracked files included) or a detached HEAD. Cherry-picks `since..from` (a merge commit in the range is refused) in a temporary detached worktree at the caller's HEAD, `<git-common-dir>/lets-integrate/<run>-<chunk>`, with hooks off; a commit that becomes empty is dropped (`--empty=drop`, git >= 2.45). The diff goes to `.lets/cache/integrate-<run>-<chunk>.patch`, is applied with `git apply --index`, and the index and worktree are verified against the picked tree. A conflict names the commit and paths and leaves the caller's tree untouched. `picked` lists the commits the pick created; `files` the paths the patch touches.
- **Temp worktree ownership.** At creation Go writes an owner marker outside it (`<git-common-dir>/lets-integrate/<run>-<chunk>.owner`: run, chunk, pid, created, path, nonce) and the same random nonce into the temp worktree's own gitdir. A leftover at that path is removed (`worktree remove --force`) only when the marker, `git worktree list --porcelain` and the nonce all match; anything else is `temp_foreign` (55) and nothing is touched. A registered temp whose directory is gone loses only its own admin dir - never a repo-wide `git worktree prune`.
- **`--revert --patch`** undoes a patch an earlier call applied, before it is committed: 58 when it no longer reverses cleanly or its paths have nothing staged, 59 when the worktree differs from the index on its paths; nothing is touched in either case. A patch whose exit-57 reverse already succeeded (`patch_path` still set) must NOT be passed to `--revert` - it would read 58.
- **Residuals.** Only hooks are off in the temp worktree: user clean/smudge filters and `core.fsmonitor` still run there.

Exit codes (50-59, the range is full; a new failure class reuses 55 with its own `error.kind`): 50 `conflict`, 51 `dirty_tree`, 52 `from_missing`, 53 `since_not_ancestor`, 54 `detached_head`, 55 `git_error` / `git_too_old` / `temp_foreign`, 56 `merge_in_range`, 57 `verify_mismatch` (revert: `revert_verify_mismatch`), 58 `revert_conflicting_edit` / `revert_nothing_staged`, 59 `revert_index_differs`; 2 usage (`--run` / `--chunk` are `[a-z0-9-]{1,40}`). Serialized by `.lets/locks/integrate.lock`.
...
- **Forward only:** a plugin older than the cached copy never replaces it (Notice: "kept vX … Not an error"). A version counts only for content provably written by LETS - the recorded key, or a copy byte-identical to an installed release; a hand-edited copy claims no version, whatever its frontmatter says, so it is saved to `.bak[-N]` and replaced (while the key names a newer plugin, only that newer session restores it). `/lets:update` asks `rulescache.Plan` - the same decision, computed without writing - before it promises that the next session start fixes the row.
...
**Session-owner guard** (merged; ships next release). A teammate pane opened in a worktree runs this same hook, so the task-state `session:` line is written only by its owner. On `startup` the boundary refresh, and on `clear` the carry to the re-minted id, run `taskstate.MaySetSession` inside the task lock (1 s deadline; a timeout is held + Notice), judged from ONE registry snapshot (`sessionGuard`, `hook_selfheal_unix.go`):

- **Refresh** writes when no well-formed session is recorded, the recorded one is dead (and not re-minted into another session), or it was re-minted into this one. A live foreign holder, or one the registry cannot judge (an unparseable live entry, a degraded read), is held.
- **Carry** (`/clear`) writes only on proof: the OLD id's peer role file (`peerscmd.RoleProof`, read before peersHeal moves it) plus `Snapshot.Rotated` showing the same process now runs this id; the sha is kept. No proof writes nothing - the residual gap when no peer role file ever recorded the id; its Notice is shown only when `.lets/sessions/peers` exists (without peers every `/clear` would say it).
- **Team worktree:** only the recorded lead (`lets members`) - or the id it was re-minted into - writes; a team file or lead record that cannot be read refuses every write and says "fix or remove the team file". An allowed pass of the lead (and a `resume`) refreshes the lead's pid.
- **No registry** (`registry_absent`, a Claude Code without a session registry): pane teammates are UNVERIFIABLE, so today's refresh is kept and a pane can still overwrite `session:` there.
- **Loud, never silent:** a declined write is a Notice naming the holder and the remedy ("run /lets:start in that chat to rewrite the session"); session-boundary then grants `prior-session`, never `exact`, and its NOTE says the range may include work that is not this session's. peersHeal gives no worker role to a session whose task-state holder is live and foreign.
