...
```bash
lets cmux open <path> [--name <slug>] [--command <cmd>] [--focus] [--force] [--json] [--quiet]
...
- **`rename`.** Relabels a cmux workspace tab (`cmux workspace rename <ref> --title`). Resolution: explicit `--ref`, else `--cwd` match against `current_directory`, else the active (`selected`) workspace. Side-effect-free (no git/files). Never hard-fails on cmux absence (`reason`: `not_macos` | `cmux_not_found` | `workspace_not_found` | `cmux_error`); only a missing `--title` is a hard error (`ExitUsage`). Lets a session stamp its own identity into the tab — directly disambiguating concurrent sessions.
