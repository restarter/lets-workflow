...
| `.claude/rules/lets-rules.md` | `drift.Check` against plugin source | Re-copy from the plugin (atomic write) on any detected drift (incl. `ahead`); `unknown` if the plugin's own frontmatter is unparseable; `in-sync` (tracks the plugin) otherwise. An `updated` row carries a past-tense detail ("was missing" / "was outdated (v…)"), never the pre-install imperative. **Scope-aware (`LETS_RULES_SCOPE`, read from the merged project>user env):** `scope=user` + missing project copy + present global = `delegated` (the project relies on `~/.claude/rules`; NOT re-created — this kills the boomerang); `scope=user` + nothing anywhere = `not-initialized` with a `lets init --user` action; a project copy present under `scope=user` gets a report-only duplication hint (never auto-deleted). Any scope value other than `user` is project semantics (fail-safe). |
