...
**`/lets:review` asks where it is.** Once, before the agents start, in the same prompt as the PR-branch question — so it's one interruption, not two. The options name what it actually found rather than asking you to type a path: your task by id and title, the newest plan file matching this branch by name, the PR description on a PR, or **no spec**. "Other" takes a task id or any path you like.
