...
- **Leave the worktree to the agent until the report is back.** Commits are matched to tasks by the files they touch, not by author - you and the agent commit under the same git identity, so a commit of yours made during the wait would be credited to a task.
