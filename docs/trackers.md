...
- **Adapters declare their fields, and `close` declares its outcome** — `create`/`set-field` open with `accepts:`, `show` with `returns:`, listing neutral field names (a native rename is written `priority`→`severity`). A command reads that declaration *before* it offers a field, so it can never collect a value from you that the tracker has nowhere to store, and never render one the tracker was not asked for. `close` declares the status it leaves the task in: `closed` when it really closes, another neutral status when this board's terminal is process-gated and the legal move was an advance — the caller then reports a handoff ("advanced to review, not closed") rather than claiming a close that never happened.
- **`## Neutral statuses` is a declaration too** — naming an optional status there (`in_review`, `blocked`) is what authorizes a command to move a task into it. `/lets:done` advances to `in_review` after opening a PR only on an adapter whose own list carries it.
...
Adapter files also carry a **verification marker on any claim the author has not exercised** — `[ASSUMED]` for something taken from documentation, `[UNVERIFIED]` for a deliberate guess, `[VERIFIED <date>]` once confirmed against the running tracker. Scoped rather than universal on purpose: a binding the author has actually run is checkable by the next reader in seconds, and a file dense with markers on self-evident lines teaches everyone to skim past them. The file is auto-loaded instruction, so an unexercised claim stated flatly is one the agent will act on.
