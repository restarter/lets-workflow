...
| `/lets:done` | Finish the task — push the branch and open a PR (`github` via `gh`, `bitbucket` via `bbb`; the task stays open until the PR merges), or merge locally and close the task (`local`). |
...
| `/lets:execute` | Execute the plan from `/lets:plan` in native plan mode: one approval, then a run-mode picker - straight-through (default), step-by-step, auto or team. `--auto` runs an approved plan without per-step gates (push/PR/`bd close`/external still gated). |
...
| Command | What it does |
|---------|--------------|
| `/lets:check` | Inline sanity check — 6 perspectives, the orchestrator alone, no subagents. Targets: working tree, staged, last commit, full branch (`--branch`, three-dot vs `origin/<merge-branch>`; the local branch only without an origin copy), a PR, `--file <path>`, `--plan`. |
