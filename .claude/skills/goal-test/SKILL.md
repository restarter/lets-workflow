...
3. **Engineer proof INTO the transcript.** Evaluator reads conversation text. If `npm test` output isn't pasted into your assistant message, evaluator doesn't see it. After every verifying step: paste the relevant output, then state the conclusion. Don't rely on "I ran the tests" — show the output line.
...
13. **Watch the evaluator's most-recent rejection reason during long runs.** Bare `/goal` shows it. If the rejection text looks like "the goal is vague" or "no concrete evidence" — your condition is mis-shaped; cancel and rewrite, don't burn more tokens.
