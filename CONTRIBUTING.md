...
4. **Declare your fields:** `create` and `set-field` open with `accepts:`, `show` with `returns:`, in NEUTRAL field names (`priority`→`severity` for a native rename). A field a caller may supply or read but the adapter never declares is one the command collects and silently drops. Keep the lists minimal - every declared field multiplies a read verb's payload. Mark each claim in the file with `[VERIFIED <date>]` / `[ASSUMED]` / `[UNVERIFIED]`; this file is auto-loaded instruction, and the agent acts on whatever it states flatly.
5. **Degradation:** OPTIONAL absent → continue with a message; a CORE verb unresolvable at runtime → HARD-FAIL loud (never a phantom success, esp. under AUTO MODE).
6. **Secrets:** never in `.lets/.env` (644, injected) or a `.board.md` (auto-loaded + git-shareable). A direct-API adapter reads its token from a gitignored chmod-0600 file (`.lets/trackers/<name>/.env`, written `0o600` + dir `0o700`); a transport that owns its own creds (an MCP server) keeps them there.
7. Optionally ship a `tracker-<name>.board.md` template (project-specific status-id map / transitions / principles) - `lets init` scaffolds it once and NEVER overwrites it.
8. The adapter contract test (`cli/internal/.../trackerrules_test.go`) auto-covers the new file (valid frontmatter + pinned header + 5 CORE rows each marked `supported = yes` (the `none` null adapter excepted) + `## Degradation` + `accepts:` on `create`/`set-field` and `returns:` on `show`, declared in neutral field names). No allowed-values list to edit - `lets init` validates `LETS_TRACKER` against `^[a-z0-9][a-z0-9-]*$` and skips with a warning if `tracker-<name>.md` is absent.
