...
- **`lets tmux open/rename/notify`** (lets-0np5i, ships next release) — the portable third `LETS_LAUNCHER`, cross-platform on unix (`cli/internal/tmuxcmd/`, `//go:build unix`; Windows stub). A file-for-file sibling of `cmuxcmd` (same JSON envelope, typed exit codes, never-hard-fail contract), driving the real `tmux` CLI. `open` opens the worktree as a **new window inside `$TMUX`** or a **new detached session** outside it (`send-keys` delivery; **never auto-attaches** — surfaces `attach_command`, because attaching would seize the Claude Code bash subprocess). Dup guard by `pane_current_path` (`EvalSymlinks`-resolved, load-bearing on macOS). `--description` is stamped into the `@lets_task` **window option**, NOT the window name (the tmux window name IS the width-budgeted status line). `notify` **broadcasts to every attached client** and reports `no_client` when none are attached — `display-message` into a detached session exits 0 while showing nobody, so a target-scoped notify would report a phantom success on the pipeline's own detached-session path. `reason` set: `tmux_not_found | tmux_error | already_open | pane_not_found | no_client`. No `not_macos`. Windows stub: `open`/`rename` hard-error, `notify` emits a parseable graceful envelope. Detail: `cli/README.md` "## lets tmux". **Not to be confused with `make dev-tmux`** (see "Local Development"): `make dev-tmux` is a **dev harness** (builds a dev binary, spawns one Claude pane per existing `.worktrees/*`, for testing the plugin); `LETS_LAUNCHER=tmux` is the **worktree launcher** (opens ONE new worktree session during `/lets:worktree create`). They share no code.
- **`lets notify`** (lets-0np5i, ships next release) — launcher-neutral gate-notification sink (`cli/internal/notifycmd/`). Resolves `LETS_LAUNCHER` (project `.lets/.env` over `~/.lets/.env` via `letsconfig.MergedEnv`) and dispatches to `cmuxcmd.Notify` / `tmuxcmd.Notify`; `terminal` and unknown values degrade to `notified=false` + a named reason. The autonomous-pipeline gate snippets (`plan-workflow.md`, `execute.md`) call `lets notify` — NOT a hardcoded `lets cmux notify` — so a tmux operator finally receives gate notifications. Launcher validation lives in `letsconfig.ShippedLaunchers` + `ValidLauncher`, pinned to the registered subcommands by `TestShippedLaunchers_MatchSubcommands` (the launcher analogue of the tracker platform's `TestShippedTrackers_MatchOnDisk` — a tracker adapter is a file on disk, a launcher is a cobra subcommand).
...
- **`/lets:update`** — sync a project with the current release. `commands/update.md` shells `lets update --json` (mirrors `/lets:init → lets init`); the Go subcommand (`cli/internal/updatecmd/`) auto-syncs `.lets/.env` (via `RegenerateEnv`, skipped on a `dev` binary) and `.claude/rules/lets-rules.md` (via `drift.Check` + re-copy), then version-checks the `lets` binary + the plugin against the latest GitHub release (report-only — can't self-replace). **Order-aware + self-driving (lets-rlue4):** when the plugin is behind (outdated vs latest, or locally < the binary) the rules/user-rules re-copy is **`deferred`** (NOT written) so rules never sync to a stale plugin — only `StateOutdated` defers (missing still installs, ahead keeps its reset). `Run` emits artifacts `.env → binary → plugin → rules → user-rules → tracker-rules` and computes a single top-level `next_action` (`init|binary|plugin|reload|done`, `command` execution-bound const for binary, `version` for done) from the artifact statuses; the command renders exactly that one step (done → `✓ Everything on vX.Y.Z`, no matrix) and can run the binary installer in-session (approval-gated, one attempt). `SchemaVersion` stays 2 (`next_action`/`deferred` are additive). Does NOT prompt for config or touch `settings.json`/beads — that's `lets init`'s job (init = setup; update = sync). Rules-drift Notices point here for `unknown`/`outdated`/`ahead`; `/lets:init` only for `missing` (global-rules drift points at `/lets:update` or `lets init --user` via `drift.MessageUser`). Scope-aware (`LETS_RULES_SCOPE`, merged project>user env): `scope=user` + missing project copy = `delegated` (rules from `~/.claude/rules`, never re-created - kills the boomerang) or `not-initialized` when nothing covers the project. The 4-artifact table (+ two optional rows: `user-rules` - omitted when `~/.claude/rules/lets-rules.md` is absent, ahead = never overwritten; and `tracker-rules` - the active `tracker-<name>.md` adapter, present when `LETS_TRACKER` names a shipped adapter (or a user-authored one with an installed copy - reported `delegated`, left as-is), and the row that applies switch cleanup + board scaffold on the edit-`.env`-then-update path) + caching detail: `cli/README.md` "## lets update" (lets-hdrdr.3).
...
- **Tracker platform** (lets-5d48z) — `LETS_TRACKER` is no longer schema-reserved: it names a **tracker adapter** (`beads` | `none`; a non-beads adapter like planfix-mcp is user-authored, not shipped). One self-contained `rules/tracker-<name>.md` per adapter binds the neutral verb set (`create`/`show`/`comment-add`/`set-status`/`close` CORE + optional `comment-list`/`list-by-status`/`search`/`ready`-stats) to a concrete transport (a `bd` command, an `mcp__*` tool, a generic REST proxy). One axis, not two — transport is encoded per-verb in the file, so there is no separate transport/driver config key. Command/skill BODIES carry neutral ` ```lets-tracker ` verb blocks (one `verb key=value` per line) — NOT inline `bd`; the orchestrator resolves each block via the loaded `tracker-<name>.md` and runs ITS binding (for beads, the same `bd` command, now table-driven + golden-pinned). A shell-computed comment/description body crosses via a `<field>-file=` temp file (`body-file=` for comments, `description-file=` for create; no computed multi-line value re-typed across the model boundary); a short or static-templated body is inline `body=`/`description=`; reads return the neutral `{id,title,status,url}` shape. `lets init` installs exactly one `tracker-<name>.md` (drift-tracked like `lets-rules.md`, auto-loaded from `.claude/rules/`). beads = reference adapter (the bd invocations are unchanged, pinned by a golden test; the lone allowlisted inline `bd` is the detect-task liveness probe, gated on the active tracker); none = null adapter (supersedes lets-nwwkj). planfix-mcp was prototyped (pure MCP via the server's generic `planfix_request` REST passthrough) but is NOT shipped - it is a user-authored adapter example (see docs/trackers.md); a popular-platform worked example is planned. A shared adapter file is trusted instruction auto-loaded into context — its bindings execute as written (review before installing one you didn't author; contract test pins shape, not safety). Adding a tracker is a one-file recipe (CONTRIBUTING "Add a tracker adapter"), enforced by a contract test + a beads-binding golden + a durable "no executable bd in a command body" test. The full platform section + the `.mcp.json` wiring live below / in `docs/trackers.md`.

## Architecture Decisions

- **Adversarial verification + Dynamic Workflows (opt-in `--workflow`).** `/lets:review` refutes every `[BLOCKER]`/`[SUGGESTION]` with `lets:skeptic` agents before reporting (asymmetric drop: SUGGESTION on a simple-majority `real=false`; BLOCKER only on near-unanimous high-confidence refute, downgraded on a simple-majority, else kept). Runs in BOTH standard (Task, capped) and `--workflow` (off-context) modes — `--workflow` is a pure performance lever, same verified findings. **SPEC injection + PR branch gate *(ships next release)*:** `/lets:review` + `/lets:check` resolve the active task's SPEC via the tracker's neutral `show` and inject it as fenced data into the reviewer prompts, so planned-but-unwired work is not flagged as scope creep; with no spec a scope finding caps at `[SUGGESTION]`. The skeptic's block is narrower - it cannot set a tier, so a tier-cap instruction would be executed as `real=false`, which `decide()` maps to a **drop**. PR mode OFFERS a `gh pr checkout` so agents read the reviewed code: it **never creates a worktree** (the user's choice; pinned by `TestReviewNeverCreatesWorktree`), `--json` never touches the working tree, and the previous branch is always restored - branch-keyed task state desyncs otherwise. `/lets:review`, `/lets:opinion`, `/lets:backlog`, `/lets:research` ship committed workflow skeletons (`skills/<name>-workflow/<name>.workflow.js`); authoring standard + runtime constraints in **Dynamic Workflow Assets** below.
...
- Agent frontmatter fields: `name`, `description`, `tools`, `color` (terminal output: red/blue/green/yellow/purple/orange/pink/cyan), optional `model`. Model policy (lets-f1lu1): agents carry **no upward model pin** - each inherits the session model, so a Fable session runs Fable everywhere (a NAME pin like `opus` goes stale and *downgrades* agents to an older model once a newer one ships - e.g. Opus 4.8 on a Fable session). The sole exception is `explorer` -> `sonnet`, a **downward cost-pin** for mechanical codebase-mapping (a downward pin can't cause a capability downgrade). Verify (`skeptic`) deliberately stays on the session model - a cheap verifier false-drops hard `[BLOCKER]`s (`decide()` drops a BLOCKER when all skeptics vote `real=false` at *any* confidence, and a weaker model leans `real=false` when it can't substantiate a deep finding). The expert/analyst agents use tiered scoring ([BLOCKER]/[SUGGESTION]/[NIT]), self-contained Modes, and Output Format sections; `explorer` (codebase mapping), `implementer` (writes code), and `skeptic` (single-finding verifier) carry their own task-specific formats instead.
...
.lets/sessions/          # Session snapshots (file-primary ## RESUME via session-snapshot; -precompact- infix for pre-compact); .task-<slug> state file (task/start/session boundaries, validated cache - lets-dsdmp); .review-restore-<session-id> (PR-mode branch/stash restore - fields `ref:`/`pr:`/optional `stash:`; session-keyed so parallel worktrees never share it; kept on any non-clean outcome, and globbed by the review gate + `/lets:start` so a stray is surfaced rather than stranded) *(ships next release)*
...
.claude/rules/tracker-<name>.md        # Active tracker adapter - plugin-MANAGED like lets-rules.md (installed/cleaned by `lets init`, synced by `/lets:update`); NEVER edit the installed copy
.claude/rules/tracker-<name>.board.md  # Board profile - USER-OWNED (scaffolded once, never overwritten); edit freely; gitignored when it holds private board data
```
...
| `LETS_TRACKER` | `.lets/.env` | Task tracker **adapter**: `beads` (default) \| `none`. Selects the one `.claude/rules/tracker-<name>.md` that `lets init` installs (one-adapter-file platform, lets-5d48z). `none` supersedes lets-nwwkj; the verb-resolution rule lives in `lets-rules.md` "Tracker Adapters". See `docs/trackers.md` |
| `LETS_LAUNCHER` | `.lets/.env` (falls back to `~/.lets/.env`) | Worktree launcher: `terminal` (default - print `cd … && claude`) \| `cmux` (cmux workspace via `lets cmux`, macOS only) \| `tmux` (tmux window/session via `lets tmux`, Linux + macOS). A preference, not a guarantee - each falls back to `terminal` when its binary/platform is absent. Validated against a whitelist pinned to the registered subcommands (`lets init --launcher=<unknown>` is rejected) |
...
**Full file-sync checklist, the Response Footer / LETS box output spec, and the per-command checklist live in `CONTRIBUTING.md` ("## Editing commands, skills, agents, config keys").** Modifying a command/skill/agent? Read it — it names every file that must stay in sync: the rules `Skill Quick Reference`, every `agents/*.md` `## Constraints` line, the `/lets:install` skill tables, `README.md`, the `end.md`/`done.md` session-id channels, and the Go-subcommand wiring.
