...
- Agent selection: each command owns its detection/selection logic (different semantics per context). Multi-agent dispatching commands (review, opinion, brainstorm, plan) show Agent Panel before launch with cost note about separate rate limits. Most agents have explicit PLAN mode for plan review.
