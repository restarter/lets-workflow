...
| `LETS_TRACKER` | `.lets/.env` | Task tracker **adapter**: `beads` (default) \| `planfix-mcp` \| `none`. Selects the one `.claude/rules/tracker-<name>.md` that `lets init` installs (one-adapter-file platform, lets-5d48z, ships next release). `none` supersedes lets-nwwkj; the verb-resolution rule lives in `lets-rules.md` "Tracker Adapters". See `docs/trackers.md` |
