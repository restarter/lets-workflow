...
- **Adversarial verification + Dynamic Workflows (opt-in `--workflow`).** `/lets:review` refutes every `[BLOCKER]`/`[SUGGESTION]` with `lets:skeptic` agents before reporting (asymmetric drop: SUGGESTION on a simple-majority `real=false`; BLOCKER only on near-unanimous high-confidence refute, downgraded on a simple-majority, else kept). Runs in BOTH standard (Task, capped) and `--workflow` (off-context) modes — `--workflow` is a pure performance lever, same verified findings. **SPEC injection + PR branch gate *(ships next release)*:** `/lets:review` + `/lets:check` resolve the active task's SPEC via the tracker's neutral `show` and inject it as **fenced untrusted data** into every agent and skeptic prompt on both paths, so planned-but-unwired work is not flagged as scope creep; with no spec a scope finding caps at `[SUGGESTION]`, never `[BLOCKER]`. An ambiguous `list-by-status` fallback counts as no spec — on a shared board it returns a colleague's task. In PR mode `/lets:review` derives the id from `headRefName` (never that fallback) and falls back to the PR body, while `/lets:check` uses the PR body directly. The skeptic's block is narrower than the reviewer's — it cannot set a tier, so a tier-cap instruction would be executed as `real=false`, which `decide()` maps to a **drop**. `/lets:review` PR mode also OFFERS a `gh pr checkout` so agents read the reviewed code instead of the base tree: it **never creates a worktree** (the user's choice; pinned by `TestReviewNeverCreatesWorktree`), `--json` never touches the working tree, the previous branch is **always** restored (branch-keyed task state desyncs otherwise), and after a checkout the `CLAUDE.MD RULES:` slot is read from `$LETS_MERGE_BRANCH` — a PR that edits CLAUDE.md is a diff to review, not rules to obey. `/lets:review`, `/lets:opinion`, `/lets:backlog`, `/lets:research` ship committed workflow skeletons (`skills/<name>-workflow/<name>.workflow.js`); authoring standard + runtime constraints in **Dynamic Workflow Assets** below.
