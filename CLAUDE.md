...
- [ ] **If file invokes `AskUserQuestion`**, follow `## AskUserQuestion Conventions` in `plugins/lets/rules/lets-rules.md` — header chip 4-12 chars descriptive (never `"LETS"` or command name), `(Recommended)` in label not description, follow-through via `Read` tool (Rule 7) when an option names a `/lets:*` command
