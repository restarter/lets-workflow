...
- **Statusline** = `lets statusline` Go subcommand. Project `.claude/settings.json` invokes it directly. `lets init` detects the canonical command via value-match against `"lets statusline"`; foreign user-customized commands are left alone. Legacy bash shim `plugins/lets/scripts/lets/statusline.sh` removed in lets-8ilsl. Byte-equal detection of pre-deletion installs (.lets/statusline.sh) handled via the frozen `cli/internal/initcmd/embedded_statusline_shim.sh` snapshot — `MigrateStatuslineSh` deletes matching legacy shims and triggers `SetStatusLine` to point settings.json at `lets statusline`. Default output is the frozen 2-line compact statusline (`renderLines`). An **experimental rich multi-line level** (`cli/internal/statusline/rich.go`) is opt-in behind `lets statusline --rich` OR env `LETS_STATUSLINE_LEVEL=rich` (ships in a later release; not yet wired into `lets init`): a closed **box** (`┌─┐ │ ├─┤ └─┘`) wrapping identity / budget / task / rotating-tip lines. Two `COLUMNS`-driven tiers — **Full** (≥`bpWide`=106) and **Compact** (<106; fails open to Full when COLUMNS is absent). The box hugs the widest line and is sized **cell-accurately** (`cellWidth`/`fitCell` count the known wide-emoji set as 2 cells) so the right border aligns — **when adding a new emoji glyph, add it to `wideRunes` or the border drifts**. Universal emoji glyphs (no Nerd Font), growth-stage brand emoji (🌱→🪴→🌿→🌳→🌴 by session `cost.total_lines_added`), color-graded effort, no progress bars (token counts + paren reset deltas instead), `--light` palette (default dark). The task line reads `.lets/cache/task-status`, self-refreshed by a detached `lets statusline --fetch-task-only` subprocess (`bd show`, 90s TTL) — no bd/network on the render path. Compact (default) is untouched. Payload-robustness: `flexISO` decodes numeric `resets_at`, and `workspace.git_worktree` is intentionally not decoded (CC sends a string) — both would otherwise blank the bar.
