...
- Agent selection: each command owns its detection/selection logic (different semantics per context). Multi-agent dispatching commands (review, opinion, brainstorm) show Agent Panel before launch with cost note about separate rate limits. Plan expert eval uses AskUserQuestion gate. 9 of 14 agents have explicit PLAN mode for plan review.
