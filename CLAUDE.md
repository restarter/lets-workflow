...
- **PreCompact hook** still runs `plugins/lets/hooks/session-start.sh` for now - port to Go is tracked separately (Phase 3.5 of lets-7vtaw, will route PreCompact to the same `lets hook session-start` subcommand and delete the .sh file).
- The bash `session-start.sh` migration block (yaml → .env) was removed from the Go port - lets-p732a closed.
