...
- **Adversarial verification + Dynamic Workflows in `/lets:review` (opt-in, ships next release).** `/lets:review` now runs an adversarial **finding-verification** step (Step 6.6) as CORE methodology: every `[BLOCKER]`/`[SUGGESTION]` is refuted by `lets:skeptic` agents (N=2, 3 for BLOCKER) before it is reported. The drop rule is **asymmetric** - a SUGGESTION drops on a simple majority `real=false`, a BLOCKER is only dropped on near-unanimous high-confidence refutation (downgraded on a simple-majority refute, otherwise kept) so verify never silently suppresses real bugs. This runs in BOTH modes: standard (`lets:skeptic` via Task tool, in-context, capped) and `--workflow` (off-context, inside the script). `--workflow` is therefore a pure **performance lever** (off-context fan-out), not a different feature: `review` and `review --workflow` produce the same verified findings (up to the standard-mode verification cap - standard mode verifies at most the top-K=5 `[SUGGESTION]`s in-context, the workflow path verifies every finding, so on a large review the workflow may verify a superset and the two can diverge). The workflow skeleton is a committed **skill-as-template** at `skills/review-workflow/review.workflow.js`, invoked via `Workflow({scriptPath: "${CLAUDE_PLUGIN_ROOT}/skills/review-workflow/review.workflow.js", args})` (NOT inline - graduated from the pilot). It runs a multi-stage off-context chain (review -> dedupe -> verify -> aggregate) and returns `{verdict, findings, systemic, summary, counts}` (`findings`+`verdict` keep the `/lets:github-pr --json` contract; `counts.refuted`/`counts.verify_failed` are additive). When `--workflow` is not passed the command prompts (Standard vs Workflow) if the `Workflow` tool is available, else uses the Task path; explicit `--workflow` always wins; preflight requires Claude Code >= 2.1.154 on a paid plan. There is no unit test for the skeleton (the runtime forbids sibling-import and the file isn't Node-importable) - validated by a live smoke test; the asymmetric rule is the drift-prone part, kept in sync with the Step 6/6.6/7 prose. See **Dynamic Workflow Assets** below for the authoring standard.
...
| Adding a **Dynamic Workflow asset** (a `Workflow`-tool script) | Follow `## Dynamic Workflow Assets (authoring standard)` above. Ship as `skills/<name>-workflow/` (`<name>.workflow.js` + `user-invocable: false` `SKILL.md`); invoke from the command via `Workflow({scriptPath: "${CLAUDE_PLUGIN_ROOT}/skills/<name>-workflow/<name>.workflow.js", args})`; obey the 6 conventions + runtime must-obey list; validate via a live smoke test (no unit test - runtime-blocked); degrade gracefully when an `agentType` can't resolve. `skills/review-workflow/` is the reference example. |
