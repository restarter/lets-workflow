...
- **Dynamic Workflows in `/lets:review` (opt-in, ships next release).** `/lets:review --workflow` runs the selected review agents inside a Claude Code Dynamic Workflow (`Workflow` tool) instead of the Task tool. Boundary: Claude owns agent selection (Step 4) + persistence/output (Steps 8-10); the inline script owns fan-out + aggregation (Steps 5-6) so per-agent findings stay off-context and only the aggregate returns. Pilot uses an INLINE script (authored by Claude from the skeleton in `commands/review.md` `## Workflow Mode`, variant B) - no committed `.js`, no JS toolchain. Agents run via `agentType: "lets:<name>"` with a `schema` lifted verbatim from the existing Step 8.5 `--json` shape (keeps the `/lets:github-pr` contract intact). When `--workflow` is not passed, the command prompts (Standard vs Workflow) for code modes if the `Workflow` tool is available, else silently uses the Task-based flow; an explicit `--workflow` always wins. Preflight requires Claude Code >= 2.1.154 on a paid plan, no silent fallback when the flag is forced. The workflow runs in the background (returns a runId; flow resumes on the completion notification). `lets:actor` is excluded from workflow mode (no PERSONALITY injection). A committed + tested workflow file is deferred to the adversarial-cross-check phase (lets-odo4o roadmap).
