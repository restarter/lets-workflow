...
| `LETS_LAUNCHER` | `.lets/.env` | Worktree launcher: `terminal` (default - print `cd … && claude`) \| `cmux` (open in a cmux workspace via `lets cmux`, macOS only). A preference, not a guarantee - `cmux` falls back to `terminal` when cmux/macOS is absent |
