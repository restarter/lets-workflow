...
- **`lets cmux open`** (ships in v0.7) — optional, macOS-only worktree launcher (`cli/internal/cmuxcmd/`, `//go:build unix`). Wraps `cmux workspace create --cwd <path>` (manaflow-ai/cmux) to open a worktree session running `claude '/lets:start <id>'`. **Strictly optional, never hard-fails:** detects cmux on PATH + `runtime.GOOS=="darwin"`; on absence/error returns `OK=true` + `launch.launched=false` + a `fallback_command`. `commands/worktree.md` Step C3.5 decides terminal-vs-cmux from `$LETS_LAUNCHER` (+ `--cmux`/`--no-cmux` override) and renders the result. **Duplicate-session guard:** `open` refuses to spawn a second workspace for a path that already has one (`reason=already_open`; `--force` overrides) — "one live session per worktree" at the launcher level. **`lets cmux rename`** relabels a workspace tab (resolve by `--ref`/`--cwd`/active) so a session can stamp its own identity (invoked on-demand by an agent/CLI, not auto-wired into a command yet) — both attack the spawner-concurrency / non-disambiguating-identity class from the cmux side (the deeper git-index/session-id mutex lives in the external spawner, not this repo). The single future home for cmux integration (e.g. `lets cmux notify`). Windows gets a stub. Detail: `cli/README.md` "## lets cmux".
...
| `LETS_LAUNCHER` | `.lets/.env` | Worktree launcher: `terminal` (default - print `cd … && claude`) \| `cmux` (open in a cmux workspace via `lets cmux`, macOS only). A preference, not a guarantee - `cmux` falls back to `terminal` when cmux/macOS is absent (ships in v0.7) |
