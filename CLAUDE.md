...
├── rules/                        #   Workflow rules (lets-rules.md) + tracker adapters (tracker-{beads,planfix-mcp,none}.md + TEMPLATE + planfix board template) - frontmatter-versioned, installed to .claude/rules/ by `lets init`
...
| `LETS_TRACKER` | `.lets/.env` | Task tracker **adapter**: `beads` (default) \| `planfix-mcp` \| `none`. Selects the one `.claude/rules/tracker-<name>.md` that `lets init` installs (one-adapter-file platform, lets-5d48z). `none` supersedes lets-nwwkj; the verb-resolution rule lives in `lets-rules.md` "Tracker Adapters". See `docs/trackers.md` |
